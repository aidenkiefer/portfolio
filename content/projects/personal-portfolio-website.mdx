---
title: "Personal Portfolio: Next.js + MDX + RAG Chatbot"
slug: "personal-portfolio-website"
date: "2025-01-01"
featured: true
tags: ["Software Architecture", "Web Architecture", "RAG Systems", "Vector Search (pgvector)", "Caching", "MDX", "Technical Writing", "SEO Fundamentals", "Accessibility", "Design Systems", "Searching", "Requirements Interpretation", "Tradeoff Analysis"]
stack: ["Next.js 16", "TypeScript", "TailwindCSS 4", "React 19", "MDX", "Supabase (pgvector)", "Upstash Redis", "Vercel"]
repoUrl: ""
liveUrl: "https://aidenkiefer.com"
summary: "A content-first portfolio and services site built with Next.js + MDX, featuring a two-stage RAG chatbot (query expansion, vector retrieval, reranking, caching) and a calm, engineer-oriented design system."
highlights:
  - "MDX case studies + typed content modules (experience, coursework, skills, services)"
  - "Two-stage RAG chatbot on /services/* (query expansion → retrieval → rerank → semantic cache)"
  - "Custom weighted search across projects, blogs, and experience (no search SaaS)"
  - "Tailwind v4 CSS-first design system with locked palette + IBM Plex typography"
  - "Accessibility-first UI patterns (dialogs, keyboard nav, live regions)"
---

## Overview

This site is a content-first portfolio and services hub built with Next.js and MDX. Instead of a “project gallery,” it’s designed to be skimmable for recruiters while still providing real engineering depth through case studies, design decisions, and evidence-backed technical writeups.

The most technically interesting feature is the services chatbot: a two-stage RAG pipeline (query expansion → vector retrieval → LLM reranking → semantic caching) that can answer questions about my services and work without exposing sensitive internals.
...
Aligned with **workflow-core** portfolio progress:

- **v1.0.2 — Release Feed (Experience)**: shipped **three curated layouts** for “Recent work” — **broadcast** (editorial cards), **command-room** (dark lateral navigator), and **matrix** (activity heatmap) — plus an **admin preview** route to compare variants.
- **v1.0.1 — Workflow bootstrap**: imported **bounded-ticket** agent docs (`docs/workflow/*`), execution rules, and a retrospective **`docs/plans/PROGRESS.md`** so shipping history stays maintainable next to the feed.
