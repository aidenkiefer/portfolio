## 01 — Enrich service page: AI Customer Service Chatbots (`/services/chatbots`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/chatbots` page.
...
- Content must be specific to AI Customer Service Chatbots and consistent with `docs/content/ai-chatbot.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
1. Use skills as above; read the enrichment spec and `docs/content/ai-chatbot.md` to understand the service and section requirements.
2. Open `app/services/chatbots/page.tsx` and locate where existing content ends and the final CTA repeat begins.
...
- `/services/chatbots` page contains all eight enrichment sections in the specified order, with the required headings.
- Existing sections (hero, problem/promise, deliverables, use cases, timeline, CTA repeat) are unchanged.
- Content is specific to AI Customer Service Chatbots and consistent with `docs/content/ai-chatbot.md`.
- Only `app/services/chatbots/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
