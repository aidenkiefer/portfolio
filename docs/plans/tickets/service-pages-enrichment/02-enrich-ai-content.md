## 02 — Enrich service page: AI Content Workflows (`/services/ai-content`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/ai-content` page.
...
- Content must be specific to AI Content Workflows and consistent with `docs/content/ai-workflows.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
- app/services/ai-content/page.tsx
...
1. Use skills as above; read the enrichment spec and `docs/content/ai-workflows.md` to understand the service and section requirements.
2. Open `app/services/ai-content/page.tsx` and locate where existing content ends and the final CTA repeat begins.
...
- `/services/ai-content` page contains all eight enrichment sections in the specified order, with the required headings.
- Existing sections (hero, problem/promise, deliverables, use cases, timeline, CTA repeat) are unchanged.
- Content is specific to AI Content Workflows and consistent with `docs/content/ai-workflows.md`.
- Only `app/services/ai-content/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
