## 03 — Enrich service page: AI Personalization (`/services/personalization`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/personalization` page.
...
- Content must be specific to AI-Powered Personalization and consistent with `docs/content/ai-personalization.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
- app/services/personalization/page.tsx
...
1. Use skills as above; read the enrichment spec and `docs/content/ai-personalization.md` to understand the service and section requirements.
2. Open `app/services/personalization/page.tsx` and locate where existing content ends and the final CTA repeat begins.
...
- `/services/personalization` page contains all eight enrichment sections in the specified order, with the required headings.
- Existing sections (hero, problem/promise, deliverables, use cases, timeline, CTA repeat) are unchanged.
- Content is specific to AI Personalization and consistent with `docs/content/ai-personalization.md`.
- Only `app/services/personalization/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
