## 04 — Enrich service page: AI Video & Voice Avatars (`/services/ai-video`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/ai-video` page.
...
- Content must be specific to AI Video & Voice Avatars and consistent with `docs/content/ai-vo-avatars.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
- app/services/ai-video/page.tsx
...
1. Use skills as above; read the enrichment spec and `docs/content/ai-vo-avatars.md` to understand the service and section requirements.
2. Open `app/services/ai-video/page.tsx` and locate where existing content ends and the final CTA repeat begins.
...
- `/services/ai-video` page contains all eight enrichment sections in the specified order, with the required headings.
- Existing sections (hero, problem/promise, deliverables, use cases, timeline, CTA repeat) are unchanged.
- Content is specific to AI Video & Voice Avatars and consistent with `docs/content/ai-vo-avatars.md`.
- Only `app/services/ai-video/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
