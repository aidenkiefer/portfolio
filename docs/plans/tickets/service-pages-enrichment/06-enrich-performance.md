## 06 — Enrich service page: Performance (`/services/performance`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/performance` page.
...
- Content must be specific to Website Speed & Performance and consistent with `docs/content/speed-perf-opt.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
1. Use skills as above; read the enrichment spec and `docs/content/speed-perf-opt.md` to understand the service and section requirements.
2. Open `app/services/performance/page.tsx` and locate where existing content ends and the final CTA repeat begins.
...
- `/services/performance` page contains all eight enrichment sections in the specified order, with the required headings.
- Existing sections (hero, problem/promise, deliverables, use cases, timeline, CTA repeat) are unchanged.
- Content is specific to Performance and consistent with `docs/content/speed-perf-opt.md`.
- Only `app/services/performance/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
