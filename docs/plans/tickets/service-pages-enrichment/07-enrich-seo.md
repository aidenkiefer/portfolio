## 07 — Enrich service page: Technical SEO (`/services/seo`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/seo` page.
...
- Content must be specific to Technical SEO Setup and consistent with `docs/content/tech-seo-setup.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
1. Use skills as above; read the enrichment spec and `docs/content/tech-seo-setup.md` to understand the service and section requirements.
2. Open `app/services/seo/page.tsx` and locate where existing content ends and the final CTA repeat begins.
...
- `/services/seo` page contains all eight enrichment sections in the specified order, with the required headings.
- Existing sections (hero, problem/promise, deliverables, use cases, timeline, CTA repeat) are unchanged.
- Content is specific to Technical SEO and consistent with `docs/content/tech-seo-setup.md`.
- Only `app/services/seo/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
