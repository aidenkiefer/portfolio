## 08 — Enrich service page: Accessibility (`/services/accessibility`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/accessibility` page.
...
- Content must be specific to Accessibility & Mobile Readiness and consistent with `docs/content/accessibility-mobile-opt.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
- app/services/accessibility/page.tsx
...
1. Use skills as above; read the enrichment spec and `docs/content/accessibility-mobile-opt.md` to understand the service and section requirements.
2. Open `app/services/accessibility/page.tsx` and locate where existing content ends and the final CTA repeat begins.
...
- `/services/accessibility` page contains all eight enrichment sections in the specified order, with the required headings.
- Existing sections (hero, problem/promise, deliverables, use cases, timeline, CTA repeat) are unchanged.
- Content is specific to Accessibility and consistent with `docs/content/accessibility-mobile-opt.md`.
- Only `app/services/accessibility/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
