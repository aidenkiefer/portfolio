## 10 — Enrich service page: Automation (`/services/automation`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/automation` page.
...
- Content must be specific to Business Process Automation and consistent with `docs/content/process-automation.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
1. Use skills as above; read the enrichment spec and `docs/content/process-automation.md` to understand the service and section requirements.
2. Open `app/services/automation/page.tsx` and locate where existing content ends and the final CTA repeat begins.
...
- `/services/automation` page contains all eight enrichment sections in the specified order, with the required headings.
- Existing sections (hero, problem/promise, deliverables, use cases, timeline, CTA repeat) are unchanged.
- Content is specific to Automation and consistent with `docs/content/process-automation.md`.
- Only `app/services/automation/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
