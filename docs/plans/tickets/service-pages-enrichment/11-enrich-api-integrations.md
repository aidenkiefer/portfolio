## 11 — Enrich service page: API Integrations (`/services/api-integrations`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/api-integrations` page.
...
- Content must be specific to API Integrations and consistent with `docs/content/api-tooling.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
- app/services/api-integrations/page.tsx
...
1. Use skills as above; read the enrichment spec and `docs/content/api-tooling.md` to understand the service and section requirements.
2. Open `app/services/api-integrations/page.tsx` and locate where existing content ends and the final CTA repeat begins.
...
- `/services/api-integrations` page contains all eight enrichment sections in the specified order, with the required headings.
- Existing sections (hero, problem/promise, deliverables, use cases, timeline, CTA repeat) are unchanged.
- Content is specific to API Integrations and consistent with `docs/content/api-tooling.md`.
- Only `app/services/api-integrations/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
