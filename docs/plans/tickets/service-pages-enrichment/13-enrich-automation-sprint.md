## 13 — Enrich package page: Automation Sprint (`/services/automation-sprint`)

## Task

- Add the **eight enrichment sections** (per `docs/plans/specs/service-pages-enrichment-spec.md`) to the existing `/services/automation-sprint` **package** page.
- Use **package framing** where the spec allows: e.g. “What This Package Actually Does,” “Who This Package Is Best For.” Real-World Examples, FAQs, and “How the Process Works” should reflect the *package* (Automation Sprint) and bundled outcomes.
- Preserve the existing hero, problem/promise, deliverables, example automations, timeline, services-in-package links, and CTA repeat. Insert the eight sections in spec order after the existing content and before the final CTA repeat.
- Content must be specific to Automation Sprint and consistent with `docs/content/automation-sprint.md`; use the global writing guidelines (non-technical founders first, outcomes, plain language).
...
- app/services/automation-sprint/page.tsx

> If a shared component for enriched sections is needed, stop and ask to extend the Allowed Files list.

## Hard Limits

- Do **not** remove or replace existing sections (hero, problem/promise, deliverables, example automations, timeline, services-in-package links, CTA repeat). Only **add** the eight enrichment sections.
...
1. Use skills as above; read the enrichment spec and `docs/content/automation-sprint.md` to understand the package and section requirements. Note: this is a **package** page—use package framing per spec (e.g. “What This Package Actually Does,” “Who This Package Is Best For”).
2. Open `app/services/automation-sprint/page.tsx` and locate where existing content ends and the final CTA repeat begins.
3. Add the eight sections in this order, using the headings and structure from the enrichment spec (with package wording where specified):
   ...
   - **Section 3:** Real-World Examples (2–3 examples tied to Automation Sprint / bundled outcomes)
...
- `/services/automation-sprint` page contains all eight enrichment sections in the specified order, with package-appropriate headings.
- Existing sections (hero, problem/promise, deliverables, example automations, timeline, services-in-package links, CTA repeat) are unchanged.
- Content is specific to Automation Sprint (package) and consistent with `docs/content/automation-sprint.md`.
- Only `app/services/automation-sprint/page.tsx` was modified; changes are summarized in ≤5 bullets or an Implementation Summary.
