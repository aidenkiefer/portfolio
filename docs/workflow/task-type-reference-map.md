...
| **Project case studies** | Sync `/projects` MDX from workflow-core progress + extraction | `docs/workflow/project-case-studies.md` | `~/projects/workflow-core/project-progress/*.md`, `~/projects/workflow-core/portfolio-extraction/*/project.md` | `content/projects/*.mdx` | `docs-agent` | `writing-plans`, `copywriting` | Stable `slug`; map `{key}` → filename; no secrets in frontmatter | HIGH | M |
